 ...
 */
	...
	virtual void render() override {
		...
		for (size_t i = 0; i < objects.size(); i++) {
			...
			GLuint lightPositionUniform = GLuint(glGetUniformLocation(obj.shaderID, "lightPos"));
			glUniform3f(lightPositionUniform, this->lightPos.x, this->lightPos.y, this->lightPos.z);
			...
			if (obj.shaderIdx == DIFFUSE_SHADER_IDX) {
				...
				GLuint rhodUniform = GLuint(glGetUniformLocation(obj.shaderID, "rho_d"));
				glUniform3f(rhodUniform, obj.rho_d.x, obj.rho_d.y, obj.rho_d.z);
				GLuint rhosUniform = GLuint(glGetUniformLocation(obj.shaderID, "rho_s"));
				glUniform3f(rhosUniform, obj.rho_s.x, obj.rho_s.y, obj.rho_s.z);
