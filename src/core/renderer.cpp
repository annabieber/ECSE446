...
void Renderer::render() {
    if (realTime) {
		...
		while (1) {
			...
			//renderpass->updateCamera(event); //Old line
			renderpass->handleEvents(event); // New line
			renderpass->render();
		...
		//clear integral buffer
		integrator->rgb->clear();

		//Add the debug option here to run threads in parallel TODO(A2)
		...
		ThreadPool::SequentialFor(0, scene.config.height, [&](int y) {
			...
			Sampler sampler(260678856 + y);

			//iterate over each pixel and calculate the color
			for (int x = 0; x < width; ++x) {
				//for (int y = 0; y < height; ++y) {
				v3f pixelColor = v3f(0, 0, 0);

				//perform uniform supersampling only when spp is requested, in this case when spp>1
				if (spp > 1) {
					for (int j = 0; j < spp; j++) {
						//scaled width with random offset
						float px = ((x + sampler.next()) * 2 / width - 1) * scaling * aspectRatio;
						//scaled height with random offset
						float py = (1 - ((y + sampler.next()) * 2 / height)) * scaling;
						//z=-1 because it's defined that way and w=0 because it represents a vector
						v4f direction(px, py, -1.f, 0.f);
						...
						//add all the colors for one pixel into a temp value
						pixelColor += integrator->render(ray, sampler);
					}
					//get the average of the color for both x and y
					pixelColor.x = pixelColor.x / spp;
					pixelColor.y = pixelColor.y / spp;
					pixelColor.z = pixelColor.z / spp;
				}
				else {
					//scaled width
					float px = ((x + 0.5) * 2 / width - 1) * scaling * aspectRatio;
					//scaled height
					float py = (1 - ((y + 0.5) * 2 / height)) * scaling;
					...
					//create a ray from the origin point of the camera and the direction calculated
					Ray ray = Ray(eye, directionWorld);
					//assign the caculated color
					pixelColor = integrator->render(ray, sampler);
				}
				//store the ray in the image buffer 
				integrator->rgb->data[(scene.config.width * y) + x] = pixelColor;
				//}
			}
		});
	}
